...
; ==============================================================================
LOAD_PALETTE_SR:
    move.l #0xC0000000, D2     ; cram address 0x0
    sub.l  #0x00200000, D2
SELECT_PALLETE_LOOP:
    add.l #0x00200000, D2      ; add address for new palette [0-3]
    dbra  D0, SELECT_PALLETE_LOOP

    move.w #0x8F02, 0x00C00004 ; set autoincrement to 2 byte
    move.l D2, 0x00C00004      ; set up vdp to write to cram address 0x0000

LOAD_PALLETE_LOOP:
    move.w (A0)+, 0x00C00000   ; move data to vdp data port
    dbra D1, LOAD_PALLETE_LOOP
